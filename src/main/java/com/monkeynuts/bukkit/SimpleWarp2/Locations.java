...
public class Locations {
	...
	public static LocationWrapper getLocation(String name, Server s) {
		Location l = null;
		Scanner scanner;
		...
		try {
			...
			try {
				...
				while (scanner.hasNextLine()) {
					...
					if (IsCorrectLength(elements) && MatchesName(elements[0], name) ){
						...
						if (IsOldWarpFormat(elements)){ // old warp format
							return new LocationWrapper(l, elements[0]);
    ...
    private static boolean IsOldWarpFormat(String[] elements) {
        return elements.length == 5;
    }

    private static boolean IsCorrectLength(String[] elements) {
        return elements.length >= 5;
    }

    private static boolean MatchesName(String element, String name) {
        return element.equalsIgnoreCase(name) || element.toLowerCase().startsWith(name.toLowerCase());
    }
